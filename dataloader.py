...
from utils import load_chipwhisperer, generate_traces, calculate_HW, load_aes_hd_ext, \
...
class Custom_Dataset(Dataset):
    def __init__(self, root = './', dataset = "Chipwhisperer", leakage = "HW",transform = None, byte = 2):
        ...
        # elif dataset == "ASCON":
        #     data_root = 'Dataset/ASCON/ascon_cw_protected.h5'
        #     target = byte
        #     (self.X_profiling, self.X_attack), (self.Y_profiling, self.Y_attack), (self.plt_profiling, self.plt_attack), self.correct_key = load_ascon(root + data_root, leakage=leakage, target = target)
            ...
            data_root = 'Dataset/ASCON/Ascon_AEAD_Initialization_first_round_dom_2shares.trs'
            (self.X_profiling, self.X_attack), (self.Y_profiling, self.Y_attack), (
                self.plt_profiling, self.plt_attack), self.correct_key = load_ascon_2(
