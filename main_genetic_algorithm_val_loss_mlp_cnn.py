...
def evaluate_ensemble(dataloaderval, models, device, ensemble, verbose=False):
    total_loss = 0
...
def evolutionary_algorithm(dataloaderval, models, device, 
    ...
    for generation in range(generations):
        ...
        for ensemble in population:
            loss = evaluate_ensemble(dataloaderval, models, device, ensemble)
            scores.append(loss)
...
if __name__ == "__main__":
    ...
    byte = 2
    num_epochs = 50
    total_num_models = 50  # 25 MLP + 25 CNN
    num_top_k_model = 10
    ...
    np.save(model_root + f"/gv.mlp_cnn_ensemble.{leakage}", 
            {
                "GE": ensemble_GE,
                "NTGE": ensemble_NTGE,
    ...
    saved_results = np.load(model_root + f"/gv.mlp_cnn_ensemble.{leakage}.npy", allow_pickle=True).item()
